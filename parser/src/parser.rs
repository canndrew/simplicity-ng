...
fn item() -> Box<dyn Parser<Output = ast::Item, Error = ParseError>> {
    item_fn().spanned().map(|item| ast::Item::ItemFn(item)).box_dyn()
}

fn item_fn() -> Box<dyn Parser<Output = ast::ItemFn, Error = ParseError>> {
    all_of((
        keyword(Keyword::FnLower).unit_then(name()),
        parens(type_field().spanned().box_dyn().comma_separated()),
        punct(PunctKind::Arrow).unit_then(prec_equal().boxed()),
        curly_braced(prec_stmt().boxed()),
    ))
    .map(|(name, args, return_ty, body)| ast::ItemFn { name, args, return_ty, body })
    .box_dyn()
}
    ...
    .spanned_unit_then(|let_span| {
        all_of((
            ...
            },
            {
                ...
                .boxed()
                .required(move || ParseError::ParseLetRhs { let_span: let_span.clone() })
            },
            {
                let let_span = let_span.clone();
        ...
        .map(|(var_pat, var_expr, body)| ast::LetStmt { var_pat, var_expr, body })
    })
    .box_dyn()
}
...
fn expr_func_type() -> Box<dyn Parser<Output = ast::ExprFuncType, Error = ParseError>> {
    all_of((
        keyword(Keyword::FnUpper).unit_then(parens(type_field().spanned().box_dyn().comma_separated())),
...
fn expr_func_term() -> Box<dyn Parser<Output = ast::ExprFuncTerm, Error = ParseError>> {
    all_of((
        keyword(Keyword::FnLower).unit_then(parens(type_field().spanned().box_dyn().comma_separated())),
...
fn func_app_arg() -> Box<dyn Parser<Output = ast::FuncAppArg, Error = ParseError>> {
    all_of((
        name(),
        punct(PunctKind::Equal).unit_then(prec_func().boxed()).optional(),
    ))
...
fn expr_match() -> Box<dyn Parser<Output = ast::ExprMatch, Error = ParseError>> {
    all_of((
        ...
        curly_braced(match_branch().spanned().box_dyn().comma_separated()),
    ))
...
fn expr_struct_type() -> Box<dyn Parser<Output = ast::ExprStructType, Error = ParseError>> {
    ...
    .unit_then(curly_braced(type_field().spanned().box_dyn().comma_separated()))
...
fn type_field() -> Box<dyn Parser<Output = ast::TypeField, Error = ParseError>> {
    name()
    .then(punct(PunctKind::Colon).unit_then(prec_equal().boxed()))
    .map(|(name, ty)| ast::TypeField { name, ty })
    .box_dyn()
}

fn expr_struct_term() -> Box<dyn Parser<Output = ast::ExprStructTerm, Error = ParseError>> {
    parens(expr_struct_term_field().spanned().box_dyn().comma_separated())
...
fn prec_app() -> Box<dyn Parser<Output = ast::PrecApp, Error = ParseError>> {
    lazy(|| {
        ...
        .then(parens(func_app_arg().spanned().box_dyn().comma_separated()).many())
...
fn prec_atom() -> Box<dyn Parser<Output = ast::PrecAtom, Error = ParseError>> {
    lazy(|| {
        any_of((
            ...
            curly_braced(prec_stmt().boxed()).map(ast::PrecAtom::Braced),
        ))
    })
    .box_dyn()
}
...
fn pat_struct_term() -> Box<dyn Parser<Output = ast::PatStructTerm, Error = ParseError>> {
    parens(pat_struct_term_field().spanned().box_dyn().comma_separated())
...
fn pat_prec_inj() -> Box<dyn Parser<Output = ast::PatPrecInj, Error = ParseError>> {
    lazy(|| pat_prec_atom().map(ast::PatPrecInj::Other))
    .box_dyn()
}
